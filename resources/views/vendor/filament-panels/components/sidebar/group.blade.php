...
<li
    ...
    {{
        $attributes->class([
            'fi-sidebar-group sh-sidebar-group',
            'fi-active' => $active,
...
>
    @if ($label)
        <div
            ...
            @endif
            class="fi-sidebar-group-btn flex items-center justify-between px-3 py-2 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400"
        >
            ...
            @if ($collapsible)
                <x-filament::icon-button
                    ...
                    class="fi-sidebar-group-collapse-btn size-4"
                />
            @endif
        </div>
    @endif
    ...
    <ul
        @if (filled($label))
            @if ($sidebarCollapsible)
                x-show="$store.sidebar.isCompact() ? ! @js($hasDropdown) : ! $store.sidebar.groupIsCollapsed(label)"
            @else
        ...
        @if ($sidebarCollapsible)
            ...
            x-transition:enter-end="fi-transition-enter-end"
        @endif
        class="fi-sidebar-group-items sh-sidebar-group-items space-y-1 mt-1"
    >
        @foreach ($items as $item)
            @php
